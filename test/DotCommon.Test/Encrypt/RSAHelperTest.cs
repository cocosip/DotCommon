using DotCommon.Encrypt;
using System.Security.Cryptography;
using Xunit;

namespace DotCommon.Test.Encrypt
{
    public class RSAHelperTest
    {
        [Fact]
        public void GenerateKeyPair_Test()
        {
            var keyPair = Netstandard21RSAHelper.GenerateKeyPair(RSAKeyFormat.PKCS1, 512);

            Assert.NotEmpty(keyPair.PublicKey);
            var encrypted1 = Netstandard21RSAHelper.EncryptAsBase64("hello", keyPair.PublicKey);
            var decrypted1 = Netstandard21RSAHelper.DecryptFromBase64(encrypted1, keyPair.PrivateKey);
            Assert.Equal("hello", decrypted1);

            var signed1 = Netstandard21RSAHelper.SignDataAsBase64("string1", keyPair.PrivateKey);
            Assert.True(Netstandard21RSAHelper.VerifyBase64Data("string1", signed1, keyPair.PublicKey));
        }

        [Fact]
        public void KeyPair_Equal_Test()
        {
            var keyPair = Netstandard21RSAHelper.GenerateKeyPair(RSAKeyFormat.PKCS8, 2048);
            var keyPair2 = new RSAKeyPair()
            {
                PrivateKey = keyPair.PrivateKey,
                PublicKey = keyPair.PublicKey
            };

            Assert.Equal(keyPair, keyPair2);
        }




        /// <summary>PKCS1与PKCS8密钥转换
        /// </summary>
        [Fact]
        public void PKCS8_PKCS1_Conver_Test()
        {
            var keyPair1 = Netstandard21RSAHelper.GenerateKeyPair(RSAKeyFormat.PKCS1);
            var pkcs8Key1 = Netstandard21RSAHelper.PKCS1ToPKCS8(keyPair1.PrivateKey);

            var encrypted1 = Netstandard21RSAHelper.EncryptAsBase64("123456", keyPair1.PublicKey);

            var decrypted1 = Netstandard21RSAHelper.DecryptFromBase64(encrypted1, keyPair1.PrivateKey);
            Assert.Equal("123456", decrypted1);

            var pkcs1Key = Netstandard21RSAHelper.PKCS8ToPKCS1(pkcs8Key1);
            Assert.Equal(keyPair1.PrivateKey, pkcs1Key);
        }


        [Fact]
        public void PKCS8KeyPair_Generate_Test()
        {
            var keyPair = Netstandard21RSAHelper.GenerateKeyPair(RSAKeyFormat.PKCS8);
            var pkcs1Key = Netstandard21RSAHelper.PKCS8ToPKCS1(keyPair.PrivateKey);
            var pkcs8Key = Netstandard21RSAHelper.PKCS1ToPKCS8(pkcs1Key);
            Assert.Equal(pkcs8Key, keyPair.PrivateKey);
        }



        [Fact]
        public void Encrypt_Decrypt_Test()
        {
            var privateKey1 = @"";
            var publicKey1 = @"";

            var str = "HelloWorld!";
            var encrypted1 = Netstandard21RSAHelper.EncryptAsBase64(str, publicKey1, RSAEncryptionPadding.Pkcs1);
            var decrypted1 = Netstandard21RSAHelper.DecryptFromBase64(encrypted1, privateKey1, RSAEncryptionPadding.Pkcs1);
            Assert.Equal(str, decrypted1);

        }

        [Fact]
        public void Sign_VerifySign_Test()
        {
            var str = "你好,中国!";
            var privateKey1 = @"";
            var publicKey1 = @"";

            var signed1 = Netstandard21RSAHelper.SignDataAsBase64(str, privateKey1, RSASignaturePadding.Pkcs1);

            Assert.True(Netstandard21RSAHelper.VerifyBase64Data(str, signed1, publicKey1));
        }

    
    }
}
