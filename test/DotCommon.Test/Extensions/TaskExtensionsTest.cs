...
namespace DotCommon.Test.Extensions
{
    public class TaskExtensionsTest
    {
        ...
        [Fact]
        public async Task TimeoutAfter_Should_Throw_Exception_When_Task_Faults()
        {
            var task = Task.FromException(new InvalidOperationException("Test exception"));
            await Assert.ThrowsAsync<InvalidOperationException>(() => task.TimeoutAfter(1000));
        }
        ...
        [Fact]
        public async Task TimeoutAfter_With_Result_Should_Not_Await_Twice_When_Task_Completes()
        {
            var task = Task.FromResult(42);
            var result = await task.TimeoutAfter(1000);
            result.ShouldBe(42);
        ...
        [Fact]
        public async Task TimeoutAfter_With_Result_Should_Throw_Exception_When_Task_Faults()
        {
            var task = Task.FromException<int>(new InvalidOperationException("Test exception"));
            await Assert.ThrowsAsync<InvalidOperationException>(() => task.TimeoutAfter(1000));
        }
